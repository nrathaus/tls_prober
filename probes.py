...
if platform.system() != "Java":
    ...
    if "socks_proxy" in os.environ:
        logging.warn(
            "Unable to honour socks_proxy environment variable, unsupported in Jython"
        )
    ...
    def process_response(self, sock):
        ...
        while True:
            ...
            try:
                ...
                response += (
                    "*(%x)" % record.version()
                )  # TODO: Not sure that recording the record layer version is worth it?
            ...
            if record.content_type() == TLSRecord.Handshake:
                ...
                while processed_bytes < record.message_length():
                    message = HandshakeMessage.from_bytes(
                        record.message()[processed_bytes:]
                    )

                    if message.message_type() == message.ServerHello:
                        response += "handshake:%s(%x)|" % (
                ...
                if alert.alert_level() == AlertMessage.Fatal:
                    response += (
                        "alert:%s:fatal|" % alert.alert_types[alert.alert_type()]
                    )
                    break
                else:
                    response += (
                ...
                if record.content_type() in record.content_types:
                    response += (
                        "record:%s|" % record.content_types[record.content_type()]
                    )
                else:
...
class NormalHandshake12PFSw13(Probe):
    ...
    def make_hello(self):
        hello = ClientHelloMessage.create(
            TLSRecord.TLS1_3, "01234567890123456789012345678901", DEFAULT_PFS_CIPHERS
        )

        record = TLSRecord.create(
            content_type=TLSRecord.Handshake,
...
class InvalidSessionID(Probe):
    ...
    def make_hello_payload(self, version, cipher_suites):
        ...
        ciphers = struct.pack(
            ">H{0}H".format(len(cipher_suites)), len(cipher_suites) * 2, *cipher_suites
        )
        hello = (
            struct.pack(
                ">H32sB", version, b"01234567890123456789012345678901", len(session_id)
            )
            + session_id
            + ciphers
            + b"\x01\x00"
    ...
    def make_hello(self, version, cipher_suites):
        ...
        hello_msg = HandshakeMessage.create(HandshakeMessage.ClientHello, hello)
...
class InvalidCiphersLength(InvalidSessionID):
    ...
    def make_hello_payload(self, version, cipher_suites):
        cipher_bytes = (
            struct.pack(">{0}H".format(len(cipher_suites)), *cipher_suites) + b"\x00"
        )
        ciphers = struct.pack(">H", len(cipher_bytes)) + cipher_bytes
        hello = (
            struct.pack(">H32sB", version, b"01234567890123456789012345678901", 0)
            + ciphers
            + b"\x01\x00"
            + b"\x00\x00"
        )
...
class EmptyChangeCipherSpec(NormalHandshake):
    ...
    def test(self, sock):
        ...
        record = TLSRecord.create(
            content_type=TLSRecord.ChangeCipherSpec,
...
class EmptyChangeCipherSpec12PFS(NormalHandshake12PFS, EmptyChangeCipherSpec12):
...
class BadHandshakeMessage(Probe):
    ...
    def make_bad_handshake(self):
        ...
        record = TLSRecord.create(
            content_type=TLSRecord.Handshake, version=TLSRecord.TLS1_0, message=content
        )
...
class Heartbeat(NormalHandshake):
    ...
    def make_heartbeat(self):
        heartbeat = HeartbeatMessage.create(HeartbeatMessage.HeartbeatRequest, b"XXXX")

        record = TLSRecord.create(
            content_type=TLSRecord.Heartbeat,
            version=self.record_version,
...
class Heartbleed(Heartbeat):
    ...
    def make_heartbeat(self):
        heartbeat = HeartbeatMessage.create(
            HeartbeatMessage.HeartbeatRequest, b"XXXX", 0x4000
        )
...
class HighTLSVersion(Probe):
    ...
    def make_hello(self):
        hello = ClientHelloMessage.create(
            settings["default_hello_version"],
            "01234567890123456789012345678901",
            DEFAULT_CIPHERS,
        )
        return hello
    ...
    def make_high_tls_hello(self):
        ...
        record = TLSRecord.create(
            content_type=TLSRecord.Handshake, version=0x400, message=hello.bytes
        )
...
class HighTLSVersion12(HighTLSVersion):
    ...
    def make_hello(self):
        hello = ClientHelloMessage.create(
            TLSRecord.TLS1_2, "01234567890123456789012345678901", DEFAULT_12_CIPHERS
        )
        return hello
...
class HighTLSVersion12PFS(HighTLSVersion):
    ...
    def make_hello(self):
        hello = ClientHelloMessage.create(
            TLSRecord.TLS1_2, "01234567890123456789012345678901", DEFAULT_PFS_CIPHERS
        )
        return hello
...
class VeryHighTLSVersion(HighTLSVersion):
    ...
    def make_high_tls_hello(self):
        ...
        record = TLSRecord.create(
            content_type=TLSRecord.Handshake, version=0xFFFF, message=hello.bytes
        )
...
class ZeroTLSVersion(HighTLSVersion):
    ...
    def make_high_tls_hello(self):
        ...
        record = TLSRecord.create(
            content_type=TLSRecord.Handshake, version=0x000, message=hello.bytes
        )
...
class HighHelloVersion(Probe):
    ...
    def make_high_tls_hello(self):
        hello = ClientHelloMessage.create(
            self.hello_version, "01234567890123456789012345678901", self.hello_ciphers
        )
...
class ZeroHelloVersion(Probe):
    ...
    def make_zero_tls_hello(self):
        hello = ClientHelloMessage.create(
            0x000, "01234567890123456789012345678901", DEFAULT_CIPHERS
        )
...
class SplitHelloRecords(Probe):
    ...
    def make_split_hello(self):
        ...
        record_one = TLSRecord.create(
            content_type=TLSRecord.Handshake,
            version=settings["default_record_version"],
            message=first,
        )
        record_two = TLSRecord.create(
            content_type=TLSRecord.Handshake, version=0x301, message=second
        )
...
class NoCiphers(Probe):
    ...
    def make_no_ciphers_hello(self):
        hello = ClientHelloMessage.create(
            self.hello_version, "01234567890123456789012345678901", []
        )
...
class SNIOneWrong(NormalHandshake):
    ...
    def make_sni_hello(self, name):
        sni_extension = ServerNameExtension.create(None, (name, "thisisnotyourname"))
...
class SNIWithDifferentType(NormalHandshake):
    ...
    def make_sni_ext(self, server_names):
        ...
        return Extension.create(extension_type=Extension.ServerName, data=ext_data)
...
class SNIOverflow(NormalHandshake):
    ...
    def make_sni_hello(self, name):
        ...
        sni_extension = Extension.create(
            extension_type=Extension.ServerName, data=ext_data
        )
...
class SecureRenegoOverflow(NormalHandshake):
    ...
    def make_secure_renego_ext(self, payload):
        secure_renego = Extension.create(
            extension_type=Extension.RenegotiationInfo, data=payload
        )
...
class MaxFragmentNull(NormalHandshake):
    ...
    def make_fragment_hello(self, payload):
        max_fragment = Extension.create(
            extension_type=Extension.MaxFragmentLength, data=payload
        )
...
class ClientCertURLsNotNull(NormalHandshake):
    ...
    def make_cert_urls_hello(self, payload):
        client_cert_url_ext = Extension.create(
            extension_type=Extension.ClientCertificateUrl, data=payload
        )
...
class TrustedCANull(NormalHandshake):
    ...
    def make_trusted_ca_hello(self, payload):
        trusted_ca_ext = Extension.create(
            extension_type=Extension.TrustedCAKeys, data=payload
        )
...
class TruncatedHMACNotNull(NormalHandshake):
    ...
    def make_truncated_hmac_hello(self, payload):
        truncated_hmac = Extension.create(
            extension_type=Extension.TruncateHMAC, data=payload
        )
...
class OCSPNull(NormalHandshake):
    ...
    def make_ocsp_hello(self, payload):
        status_request = Extension.create(
            extension_type=Extension.StatusRequest, data=payload
        )
...
class DoubleExtension(NormalHandshake):
    ...
    def make_double_ext_hello(self):
        secure_renego = Extension.create(
            extension_type=Extension.RenegotiationInfo, data="\x00"
        )

        record = self.make_hello([secure_renego, secure_renego])
        return record
...
class UserMappingNull(NormalHandshake):
    ...
    def make_user_mapping_ext(self, value):
        user_mapping_ext = Extension.create(extension_type=6, data=value)
...
class ClientAuthzNull(NormalHandshake):
    ...
    def make_client_authz_hello(self, value):
        client_authz_ext = Extension.create(extension_type=7, data=value)
...
class ServerAuthzNull(ClientAuthzNull):
    ...
    def make_client_authz_hello(self, value):
        server_authz_ext = Extension.create(extension_type=8, data=value)
...
class CertTypeNull(NormalHandshake):
    ...
    def make_cert_type_hello(self, value):
        cert_type_ext = Extension.create(extension_type=9, data=value)
...
class SupportedGroupsNull(NormalHandshake):
    ...
    def make_supported_groups_hello(self, value):
        supported_groups_ext = Extension.create(extension_type=10, data=value)
...
class SupportedGroupsOverflow12PFS(SupportedGroupsOverflow, NormalHandshake12PFS):
...
class ECPointFormatsNull(NormalHandshake):
    ...
    def make_point_formats_hello(self, value):
        point_formats_ext = Extension.create(extension_type=11, data=value)
...
class ECPointFormatsOverflow12PFS(ECPointFormatsOverflow, NormalHandshake12PFS):
...
class ECPointFormatsCompOnly12PFS(ECPointFormatsCompOnly, NormalHandshake12PFS):
...
class SRPNull(NormalHandshake):
    ...
    def make_srp_hello(self, value):
        srp_ext = Extension.create(extension_type=12, data=value)
...
class SigAlgsNull(NormalHandshake):
    ...
    def make_signature_alg_hello(self, value):
        sig_algs_ext = Extension.create(extension_type=13, data=value)
...
class SigAlgsOddLen(SigAlgsNull):
    ...
    def test(self, sock):
        ...
        sock.write(
            self.make_signature_alg_hello(b"\x00\x05" b"\x04\x01" b"\x04\x03\x04")
        )
...
class SigAlgsOverflow(SigAlgsNull):
    ...
    def test(self, sock):
        ...
        sock.write(self.make_signature_alg_hello(b"\x00\x06" b"\x04\x01" b"\x04\x03"))
...
class UseSrtpNull(NormalHandshake):
    ...
    def make_use_srtp_hello(self, value):
        use_srtp_ext = Extension.create(extension_type=14, data=value)
...
class UseSrtpOddLen(UseSrtpNull):
    ...
    def test(self, sock):
        ...
        sock.write(
            self.make_use_srtp_hello(b"\x00\x05" b"\x00\x01\x00\x05\x00" b"\x00")
        )
...
class UseSrtpOverflow(UseSrtpNull):
    ...
    def test(self, sock):
        ...
        sock.write(self.make_use_srtp_hello(b"\x00\x06" b"\x00\x01\x00\x05"))
...
class HeartbeatNull(NormalHandshake):
    ...
    def make_heartbeat_hello(self, value):
        heartbeat_ext = Extension.create(extension_type=15, data=value)
...
class ALPNNull(NormalHandshake):
    ...
    def make_alpn_hello(self, value):
        alpn_ext = Extension.create(extension_type=16, data=value)
...
class ALPNUnknown(ALPNNull):
    ...
    def test(self, sock):
        ...
        sock.write(self.make_alpn_hello(b"\x00\x08" b"\x07unknown"))
...
class ALPNOverflow(ALPNNull):
    ...
    def test(self, sock):
        ...
        sock.write(self.make_alpn_hello(b"\x00\x10" b"\x08http/1.1"))
...
class OCSPv2Null(NormalHandshake):
    ...
    def make_ocspv2_hello(self, value):
        ocspv2_ext = Extension.create(extension_type=17, data=value)
...
class OCSPv2Overflow(OCSPv2Null):
    ...
    def test(self, sock):
        logging.debug("Sending Client Hello...")
        data = (
            b"\x00\x10"  # overall length of extension (too large)
            ...
            b"\x00\x00"  # responder ID list length
            b"\x00\x00"
        )  # request extensions list length
...
class SignedCertTSNotNull(NormalHandshake):
    ...
    def make_signed_cert_ts_hello(self, value):
        signed_cert_ts_ext = Extension.create(extension_type=18, data=value)
...
class ClientCertTypeNull(NormalHandshake):
    ...
    def make_client_cert_type_hello(self, value):
        client_cert_type_ext = Extension.create(extension_type=19, data=value)
...
class ClientCertTypeOverflow12PFS(ClientCertTypeOverflow, NormalHandshake12PFS):
...
class ServerCertTypeNull(NormalHandshake):
    ...
    def make_server_cert_type_hello(self, value):
        server_cert_type_ext = Extension.create(extension_type=20, data=value)
...
class ServerCertTypeOverflow12PFS(ServerCertTypeOverflow, NormalHandshake12PFS):
...
class PaddingNull(NormalHandshake):
    ...
    def make_padding_hello(self, value):
        padding_ext = Extension.create(extension_type=21, data=value)
    ...
    def make_padding_hello(self, value):
        ...
        records = [
            TLSRecord.create(
                content_type=TLSRecord.Handshake,
                version=TLSRecord.TLS1_0,
                message=hello_bytes[i : i + RECORD_MAX],
            ).bytes
...
class EtMNotNull(NormalHandshake):
    ...
    def make_etm_hello(self, value):
        etm_ext = Extension.create(extension_type=22, data=value)
...
class EMSNotNull(NormalHandshake):
    ...
    def make_ems_hello(self, value):
        ems_ext = Extension.create(extension_type=23, data=value)
...
class CachedInfoNull(NormalHandshake):
    ...
    def make_cached_info_hello(self, value):
        cached_info_ext = Extension.create(extension_type=25, data=value)
...
class CachedInfoOverflow(CachedInfoNull):
    ...
    def test(self, sock):
        ...
        sock.write(
            self.make_cached_info_hello(
                b"\x00\x44"
                b"\x02"  # type cert_req
                b"\x20" + b"\xaf" * 32  # length of hash
            )
        )  # hash
...
class SessionTicketNull(NormalHandshake):
    ...
    def make_session_ticket_hello(self, value):
        session_ticket_ext = Extension.create(extension_type=35, data=value)
...
class SessionTicketOverflow(SessionTicketNull):
    ...
    def test(self, sock):
        ...
        sock.write(self.make_session_ticket_hello(b"\x02\x00" + b"\xe7" * 0xFF))
...
class NPNNotNull(NormalHandshake):
    ...
    def make_npn_hello(self, value):
        npn_ext = Extension.create(extension_type=13172, data=value)
...
class TACKNotNull(NormalHandshake):
    ...
    def make_tack_hello(self, value):
        tack_ext = Extension.create(extension_type=62208, data=value)
