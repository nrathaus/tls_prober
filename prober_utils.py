...
def make_hello(extensions=tuple()):
    ...
    record = TLSRecord.create(
        content_type=TLSRecord.Handshake, version=TLSRecord.TLS1_0, message=hello.bytes
    )
...
def make_hello_request(version=TLSRecord.TLS1_0):
    hello_req = HandshakeMessage.create(HandshakeMessage.HelloRequest, b"")
    record = TLSRecord.create(
        content_type=TLSRecord.Handshake, version=version, message=hello_req.bytes
    )
    return record.bytes
...
def make_ccs(version=TLSRecord.TLS1_0):
    ...
    record = TLSRecord.create(
        content_type=TLSRecord.ChangeCipherSpec, version=version, message=ccs
    )
...
class Extension(TLSExtension):
    ...
    def create(cls, extension_type, data, length=-1):
        ...
        self.bytes = struct.pack("!HH%ds" % (len(data)), extension_type, length, data)
        return self
